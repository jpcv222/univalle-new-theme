...
<script>
    ...
    var galleyListpdf = document.getElementsByClassName("pdf");

      for (var i=0; i<galleyListpdf.length; i++) {
		var text = galleyListpdf[i].textContent;
        if( text.includes("Español (España)")){
			galleyListpdf[i].innerHTML = "PDF (Español)";
		}
       }

    var galleyList = document.getElementsByClassName("obj_galley_link");

      for (var i=0; i<galleyList.length; i++) {
		var text = galleyList[i].textContent;
        if( text.includes("Español (España)")){
			galleyList[i].innerHTML = "Texto (Español)";
		}
       }
